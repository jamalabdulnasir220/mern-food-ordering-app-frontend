import { useGetRestaurantManagers, useUpdateApplicationStatus, useGetAllRestaurants, useUpdateRestaurantApprovalStatus } from "@/api/adminApi";
...
const AdminDashboardPage = () => {
  ...
  const { restaurants, isLoading: isLoadingRestaurants } = useGetAllRestaurants();
  const { updateApprovalStatus, isLoading: isUpdatingApproval } = useUpdateRestaurantApprovalStatus();
  
  const [activeTab, setActiveTab] = useState<"managers" | "restaurants">("managers");
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {managers && managers.map((manager: User) => (
                  <tr key={manager._id}>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {restaurants && restaurants.map((restaurant: Restaurant) => (
                  <tr key={restaurant._id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{restaurant.restaurantName}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <div>{getManagerName(restaurant)}</div>
                      <div className="text-xs text-gray-400">{getManagerEmail(restaurant)}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{restaurant.city}, {restaurant.country}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                        restaurant.approvalStatus === 'approved' ? 'bg-green-100 text-green-800' : 
                        restaurant.approvalStatus === 'rejected' ? 'bg-red-100 text-red-800' : 
                          'bg-yellow-100 text-yellow-800'
                      }`}>
