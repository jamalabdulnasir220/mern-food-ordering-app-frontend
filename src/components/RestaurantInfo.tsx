...
const RestaurantInfo = ({ restaurant }: Props) => {
  return (
    <Card className="border-none shadow-lg bg-gradient-to-br from-orange-50 to-white p-2 sm:p-4">
      <CardHeader className="space-y-1 pb-2 px-1 sm:px-2">
        <div className="flex items-center gap-2 sm:gap-3 mb-2">
          <CardTitle className="text-xl sm:text-3xl font-extrabold text-orange-600 tracking-tight drop-shadow-sm">
      ...
      <CardContent className="flex flex-wrap items-center gap-2 sm:gap-3 py-1 px-1 sm:py-2 sm:px-2">
        ...
        <span className="flex items-center gap-1 sm:gap-2 flex-wrap ml-0 sm:ml-2 w-full sm:w-auto">
          {restaurant.cuisines.map((cuisine, index) => (
            <span
              key={cuisine}
              className="inline-flex items-center px-2 py-0.5 sm:py-1 bg-orange-100 rounded-full text-orange-800 text-xs sm:text-sm font-semibold shadow-sm mr-1 sm:mr-2 mb-1"
            >
              {cuisine}
